...
{
    public class IsTheIndexOfAnExistingAccount : IArgumentValidator
    {
        ...
        public Notification Check(ExecutionArguments executionArguments, int argumentIndex)
        {
            ...
            if (!statement.Accounts.GetIndexedValues().Select(x => x.Index.ToString()).Any(x => x == argument))
            {
                return Notification.ErrorFor(IndexDoesNotExistMessageText, typeof(Account).GetSingularUIDescription());
            }
