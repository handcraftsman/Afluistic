...
{
    public class IsTheNameOfAnExistingAccountType : IArgumentValidator
    {
        ...
        public Notification Check(ExecutionArguments executionArguments, int argumentIndex)
        {
            ...
            if (!statement.AccountTypes.Any(x => x.Name == argument))
            {
                return Notification.ErrorFor(NameDoesNotExistMessageText, typeof(AccountType).GetSingularUIDescription());
            }
